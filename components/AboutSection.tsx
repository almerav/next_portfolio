...
case 'Skills and Tools':
  ...
  return (
    <section id="about" className="min-h-screen bg-[#0a0a0a] px-4 sm:px-6 md:px-8 py-20 text-white">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-10 w-full">

          {/* Mobile Horizontal Tabs with Icons */}
          ...
          <div className="hidden md:block md:w-1/4 space-y-4">
            ...
            {tabs.map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`w-full text-left px-4 py-2 rounded-md font-semibold transition-all duration-300 transform hover:scale-105 ${
                  activeTab === tab
                    ? 'bg-pink-400 text-gray-900'
                    : 'bg-gray-800 text-white hover:bg-pink-200 hover:text-gray-800'
                }`}
          ...
          {/* Right Content */}
          <div className="md:w-3/4 w-full">{renderContent()}</div>
        </div>
      </section>
